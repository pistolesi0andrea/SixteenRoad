...
function desktopFilterChipClass(isActive: boolean) {
...
function desktopFieldButtonClass(isActive: boolean) {
...
export function CollectionCatalogView({
  ...
  return (
    <div className="bg-brand-cream">
      <section className="border-b border-brand-border bg-brand-cream px-5 py-8 sm:px-8 sm:py-10 md:px-10 md:py-14 xl:px-14 xl:py-16">
        <div className="flex flex-col gap-8">
          <div className="flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="min-h-[56px] w-full border border-brand-border bg-white px-6 text-[14px] uppercase tracking-[0.06em] text-brand-dark-brown transition-colors duration-200 hover:bg-brand-parchment sm:min-h-[64px] sm:min-w-[190px] sm:w-auto sm:px-8 sm:text-[17px] sm:tracking-[0.08em] lg:hidden"
              >
              ...
              <button
                ...
                className="hidden min-h-[64px] min-w-[190px] border border-brand-border bg-white px-8 text-[17px] uppercase tracking-[0.08em] text-brand-dark-brown transition-colors duration-200 hover:bg-brand-parchment lg:block"
              >
                Filtri {activeFilterCount(currentFilters) > 0 ? `(${activeFilterCount(currentFilters)})` : ""}
              </button>
            </div>
          ...
          <form
            ...
            className="hidden border border-brand-border bg-brand-parchment p-4 lg:block"
          >
            <div className="grid grid-cols-1 gap-3 xl:grid-cols-[1.2fr_1fr_0.7fr_0.7fr_auto_auto]">
          ...
          {isFiltersOpen ? (
            <div className="overflow-hidden border border-brand-border bg-brand-parchment lg:hidden">
      ...
      <div
        className={`fixed inset-0 z-[460] hidden transition-opacity duration-300 lg:block ${
      ...
      >
        ...
        >
          <div className="flex items-center justify-between border-b border-brand-border px-5 py-6 sm:px-7 sm:py-8 md:px-9">
            <h2 className="font-josefin text-[36px] leading-none text-brand-dark-brown sm:text-[44px] md:text-[52px]">Filtri</h2>
            <button
              type="button"
              onClick={() => setIsDrawerOpen(false)}
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {sortOptions.map((option) => (
                  <button
                    ...
                    className={desktopFilterChipClass(draftFilters.sort === option.value)}
                  >
                    {option.label}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                {facets.categories.map((category) => (
                  <button
                    ...
                    type="button"
                    onClick={() => toggleDraftField("category", category)}
                    className={desktopFieldButtonClass(draftFilters.category === category)}
                  >
                    {category}
                  </button>
                ))}
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                {facets.colors.map((color) => (
                  <button
                    ...
                    type="button"
                    onClick={() => toggleDraftField("color", color)}
                    className={desktopFieldButtonClass(draftFilters.color === color)}
                  >
                    {color}
                  </button>
                ))}
                ...
                <div className="grid grid-cols-4 gap-3 md:grid-cols-5">
                  {sizeGroups.apparel.map((size) => (
                    <button
                      ...
                      type="button"
                      onClick={() => toggleDraftField("size", size)}
                      className={desktopFieldButtonClass(draftFilters.size === size)}
                    >
                      {size}
                    </button>
                  ))}
          ...
          <div className="sticky bottom-0 border-t border-brand-border bg-brand-pale px-5 py-4 sm:px-7 sm:py-5 md:px-9">
            <div className="flex flex-col gap-3 md:flex-row">
              ...
              <button
                ...
                className="flex-1 bg-brand-dark-brown px-6 py-4 text-[11px] uppercase tracking-[0.18em] text-brand-cream sm:text-[12px] sm:tracking-[0.22em]"
              >
                {isPending ? "Aggiorno..." : "Applica filtri"}
              </button>
            </div>
          </div>
        </aside>
